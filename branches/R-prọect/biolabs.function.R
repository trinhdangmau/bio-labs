...
count.locality.acc<-function(localityid){
  drv<-dbDriver("SQLite")
  con<-dbConnect(drv, datafile())
  sql<-paste("SELECT specieid, count50, (count50 + count100) AS sum100, (count50+count100+count150) AS sum150, (count50+count100+count150+count200) AS sum200, count250, count300, count400, count500, count700, count1000 FROM fullspecimen WHERE localityid = '", localityid, "'", sep="")
...
ignore.locality<-function(locationid){
  if(locationid=="TT"){
    ignore<-c("V0064", "V0065", "V0066", "V0243")
  }else{
    if(locationid=="TH"){
      ignore<-c("V0007", "V0032")
    }else{
      if(locationid=="BT"){
        ignore<-c("V0102", "V0120")
      }
    }
  }
  ignore
